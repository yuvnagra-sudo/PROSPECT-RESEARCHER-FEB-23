...
- Output ONLY the instruction text, no explanation, no quotes, no JSON`;
  ...
  if(req.method==='GET'&&p.match(/^\/api\/export\/\d+$/)){const jid=parseInt(p.split('/').pop());const job=S.gJ.get(jid);
    ...
    if(job.provider==='pagespeed'){
      ...
      const fixedHdrs=['Company','URL','Final URL','Status','Summary',
        ...
        'Response Ms','HTTP Status','HTTPS Works','HTTP→HTTPS Redirect','Redirect Hops',
      ...
      for(let offset=0;offset<totalRowCount;offset+=PAGE){
        ...
        for(const r of rows){
          ...
          const cols=[
            ...
            ms(m.responseMs),m.status||'',yn(m.httpsWorks),yn(m.httpRedirects),m.redirectHops??'',
  ...
  if(req.method==='GET'&&p.match(/^\/api\/bulk-audit-csv\/\d+$/)){
    ...
    const headers=[
      ...
      'X-Content-Type-Options','X-Frame-Options','HSTS','JS Rendered',
    ...
    const csvRows=rows.map(r=>{
      ...
      const row=[
        ...
        m.responseMs??'',m.status??'',m.httpsWorks??'',m.httpRedirects??'',m.redirectHops??'',
        ...
        m.xContentType??'',m.xFrame??'',m.hsts??'',m.jsRendered?'Yes':'No',
